...
export default function Editor({ onBack, user }) {
  ...
  useEffect(() => {
    if (!editingId && !creating) {
      const url = isJournalist ? '/api/articles?mine=true' : '/api/articles'
      apiFetch(url).then(r => r.json()).then(setArticles)
    }
  }, [editingId, creating])

  if (creating) {
    return <ArticleEditor articleId={null} user={user} onBack={() => setCreating(false)} />
  }

  if (editingId !== null) {
    return <ArticleEditor articleId={editingId} user={user} onBack={() => setEditingId(null)} />
  }
      ...
      <div className="editor-article-list">
        {articles.length === 0 && (
          <p className="editor-empty">
            {isJournalist ? 'Nu ești asignat la niciun articol.' : 'Nu există articole.'}
          </p>
        )}
        {articles.map(a => (
